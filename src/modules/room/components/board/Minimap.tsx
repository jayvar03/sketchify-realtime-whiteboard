...
const MiniMap = ({ dragging }: { dragging: boolean }) => {
  ...
  useEffect(() => {
    const unsubX = miniX.on("change", (newX) => {
      if (!dragging && draggingMinimap) boardPos.x.set(Math.floor(-newX * divider));
    });
    const unsubY = miniY.on("change", (newY) => {
      if (!dragging && draggingMinimap) boardPos.y.set(Math.floor(-newY * divider));
    });
  ...
  }, [boardPos.x, boardPos.y, divider, dragging, draggingMinimap, miniX, miniY]);
